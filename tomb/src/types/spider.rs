...
impl SpiderMetadata {
    ...
    pub fn new(path_root: &Path, entry: DirEntry<((), ())>) -> Self {
        ...
        let original_location = entry.path().strip_prefix(path_root).expect("failed to strip prefix").to_path_buf();
        // Don't try to canonicalize if this is a symlink
        let canonicalized_path: PathBuf = if entry.path_is_symlink() {
            entry.path()
        } else {
            entry.path().canonicalize().expect("failed to canonicalize path")
        };
