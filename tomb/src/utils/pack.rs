use anyhow::Result;
use tomb_common::types::{keys::manager::Manager, blockstore::tombblockstore::TombBlockStore};
use std::{
    collections::HashSet,
    path::{Path, PathBuf},
    rc::Rc,
...
use wnfs::{
    common::{BlockStore as WnfsBlockStore, dagcbor},
    private::{PrivateDirectory, PrivateFile, PrivateForest}, libipld::{Cid, IpldCodec},
};
...
pub async fn store_keys(
    ...
    let cid1 = metadata.put_block(bytes.clone(), IpldCodec::Raw).await?;
    let cid2 = content.put_block(bytes, IpldCodec::Raw).await?;
    assert_eq!(cid1, cid2);
    Ok(cid1)
}

pub async fn update_key_content(
    manager: &Manager,
    manager_cid: Cid,
    ...
    let bytes = dagcbor::encode(&manager)?;
    let cid1 = metadata.update_content(&manager_cid, bytes.clone(), IpldCodec::Raw).await?;
    let cid2 = content.update_content(&manager_cid, bytes, IpldCodec::Raw).await?;
    assert_eq!(cid1, cid2);
    Ok(cid1)
}
