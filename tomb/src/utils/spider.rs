...
pub async fn spider(
    ...
    for spidered in spidered.into_iter() {
        ...
        if spidered.original_metadata.is_dir() {
            ...
            bundleing_plan.push(BundlePipelinePlan::Directory(origin_data));
        }
        // If this is a symlink
            ...
            bundleing_plan.push(BundlePipelinePlan::Symlink(origin_data, symlink_target));
        }
        // If this is a file that was not in a group
        else {
            // Push a BundlePipelinePlan using fake file group of singular spidered metadata
            bundleing_plan.push(BundlePipelinePlan::FileGroup(vec![origin_data]));
        }
    }
    Ok(bundleing_plan)
}
