...
mod tests {
    ...
    fn test_pem_key_parse_and_use() {
        let private_key =
            EcEncryptionKey::import(TEST_PEM_KEY).expect("parsing known good pem key to succeed");
        let protected_key = EncryptedSymmetricKey::import(SEALED_KEY)
            .expect("parsing known good sealed key to succeed");
        let plain_key = protected_key
    ...
    #[test]
    fn test_der_key_parse_and_use() {
        let private_key = EcEncryptionKey::import_bytes(TEST_DER_KEY)
            .expect("parsing known good der key to succeed");
        let protected_key = EncryptedSymmetricKey::import(SEALED_KEY)
            .expect("parsing known good sealed key to succeed");
        let plain_key = protected_key
            .decrypt_with(&private_key)
