...
fn carv1_header(metadata_cid: &[u8], wnfs_cid: &[u8]) -> Vec<u8> {
    let mut header_bytes = vec![
        // This includes specific documentation around the CARv1 format as well, trying to
        // represent as many features and variants as possible for our conformance tests.

        // We have an unsigned LEB128 variable integer starting us off. This indicates the total
        ...
        // > splitting for major types.
        //
        // Since our strings are different lengths, the first byte indicating the type + length is
        // sufficient to determine the order the keys will appear in "root" is a length of 5, and
        // "version" is a length of 7 so "roots" should always appear first.
        ...
        // the main data segment that exist in the CARv2 index but not here and are unrelated to
        // us).
        //
        // An array is Major Type 4 or 0b100, and this has a "short-count" equal to the number of
        // elements we have which is 2 or 0b0_0010, giving us:
        0x82,
    ];

    // The specification allows for both CIDv0 and CIDv1 to be present here. Array ordering of the
    // roots doesn't matter to the spec, to use its a useful convention to include our out of tree
    // metadata and the filesystem later on for invalid format rejection on upload.
    header_bytes.extend_from_slice(metadata_cid);
    header_bytes.extend_from_slice(wnfs_cid);
...
fn build_full_car() -> Vec<u8> {
    ...
    /// CARv1 Header
    // let data_start = all_car_bytes.len();
    let header = Header {
        version: 2,
