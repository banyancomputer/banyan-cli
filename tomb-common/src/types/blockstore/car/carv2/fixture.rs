    ...
    fn build_full_car() -> Vec<u8> {
        ...
        let header = Header {
            ...
            roots: RefCell::new(vec![
                Cid::from_str(&binary_cid_to_base58_cid(&block_zero_cid)).expect("failed to represent binary as CID"),
                Cid::from_str(&binary_cid_to_base58_cid(&block_five_cid)).expect("failed to represent binary as CID"),
            ]),
        };
        let header_bytes = header.to_ipld_bytes().expect("failed to convert header to IPLD");
