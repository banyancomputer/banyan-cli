...
impl Mapper {
    ...
    pub fn update_temporal_key(&mut self, new_key: &TemporalKey) -> Result<()> {
        ...
        for (fingerprint, (der, _)) in self.0.clone() {
            ...
            let protected_key = symmetric.encrypt_for(&public_key)?;
    ...
    pub fn insert_public_key(
        ...
        let fingerprint = pretty_fingerprint(&new_key.fingerprint()?);
        // Represent the public key as DER bytes
        let der = new_key.export_bytes()?;
        ...
        if let Some(temporal_key) = temporal_key {
            ...
            // Encrypt the bytes
            let protected_key = symmetric.encrypt_for(new_key)?;
    ...
    pub fn reconstruct(&self, private_key: &EcEncryptionKey) -> Result<TemporalKey> {
        // Grab the fingerprint
        let fingerprint = pretty_fingerprint(&private_key.fingerprint()?);
...
mod test {
    ...
    async fn to_from_ipld() -> Result<()> {
        ...
        // Insert a public key
        mapper1.insert_public_key(&None, &public_key)?;
        ...
        // Update temporal key
        mapper2.update_temporal_key(&temporal_key)?;
        ...
        // Assert decryption
        let new_temporal_key = mapper3.reconstruct(&wrapping_key)?;
