...
impl EncryptedPrivateRef {
    ...
    pub async fn encrypt_for(
        ...
        let encrypted_temporal_key = 
            temporal_key.encrypt_for(recipient_key)
            .await
    ...
    pub async fn decrypt_with(
        ...
        let encrypted_temporal_key = EncryptedSymmetricKey::import(&self.encrypted_temporal_key_string)
            .expect("could not import encrypted temporal key");
        // Decrypt the encrypted temporal key with the recipient key
        let temporal_key = encrypted_temporal_key.decrypt_with(recipient_key)
            .await
