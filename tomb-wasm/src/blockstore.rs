...
use tomb_common::types::blockstore::{car::carv2::CAR as CarV2, tombblockstore::TombBlockStore};
use wnfs::{
...
impl CarV2BlockStore {
    ...
    pub async fn from_vec(vec: Vec<u8>) -> Result<CarV2BlockStore, TombWasmError> {
        // Read data
        let data = vec;
        // Load car
        let car = CarV2::read_bytes(Cursor::new(&data)).unwrap();
        // Ok
        Ok(Self { data, car })
    }
}
