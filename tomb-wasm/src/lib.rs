...
use tomb_common::banyan::{
    credentials::Credentials as BanyanCredentials,  
    client::Client as BanyanClient
};
...
impl TombWasm {
    ...
    pub fn new(web_signing_key: CryptoKey, account_id: String, api_endpoint: String) -> Self {
        set_panic_hook();
        log!("tomb-wasm: new()");
        let mut banyan_client = BanyanClient::new(&api_endpoint).unwrap();
        ...
        let banyan_credentials = BanyanCredentials{
            account_id,
            signing_key,
        };
        // log!("tomb-wasm: banyan_credentials: {:?}", JsValue::from_serde(&banyan_credentials).unwrap());
